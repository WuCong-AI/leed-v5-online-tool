"""Customized multi-version LEED design guide generation."""
...
from ..data import CERTIFICATION_THRESHOLDS, credits_for, prerequisites_for, scorecard_total
...
def strategic_credits(profile: ProjectProfile) -> tuple[Credit, ...]:
    ...
    total = scorecard_total(profile.project_type, profile.rating_version)
    threshold = CERTIFICATION_THRESHOLDS[profile.target_level]
    goal = min(total, threshold + (8 if profile.target_level in {"Gold", "Platinum"} else 5))
    category_priority = {
        "Energy & Atmosphere": 0, "Water Efficiency": 1,
        "Indoor Environmental Quality": 2, "Materials & Resources": 3,
        "Location & Transportation": 4, "Sustainable Sites": 5,
        "Integrative Process": 6, "Innovation": 7, "Regional / Project Priority": 8,
    }
    ranked = sorted(credits, key=lambda c: (category_priority.get(c.category, 9), -c.points, c.code))
    selected: list[Credit] = []
    points = 0
...
def _platinum_note(profile: ProjectProfile) -> str:
    ...
    if profile.rating_version != "LEED v5":
        return "Platinum requires 80+ points and every applicable prerequisite; verify registration-date addenda and minimum program requirements."
    return (
...
def build_guide_markdown(profile: ProjectProfile) -> str:
    ...
    threshold = CERTIFICATION_THRESHOLDS[profile.target_level]
    target_with_buffer = min(scorecard_total(profile.project_type, profile.rating_version), threshold + (8 if profile.target_level in {"Gold", "Platinum"} else 5))
    lines = [
        f"# {profile.rating_version} Design Guide — {profile.name}",
        "",
        f"**Project:** {profile.name}  ",
        f"**Location:** {profile.location}  ",
        f"**Gross floor area:** {profile.gross_floor_area_m2:,.0f} m²  ",
        f"**Rating system:** {profile.project_type}  ",
        f"**Version:** {profile.rating_version}  ",
...
<title>{escape(profile.rating_version)} Design Guide — {escape(profile.name)}</title>
<style>
...
<header><div class="eyebrow">{escape(profile.rating_version)} design guide</div><h1>{escape(profile.name)}</h1>
