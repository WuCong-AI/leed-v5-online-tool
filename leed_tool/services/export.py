...
def build_summary_html(
    ...
    return f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><title>LEED v5 Automated Review</title>
...
<body><main><header><small>UPLOAD-FIRST AUTOMATED REVIEW</small><h1>{escape(project_name)}</h1><p>Planning evidence screen; not a certification decision.</p></header>
...
def build_result_bundle(
    ...
    html = build_summary_html(project_name, documents, credit_evidence, prerequisites, findings, projection)
    output = BytesIO()
    with ZipFile(output, "w", compression=ZIP_DEFLATED) as archive:
        archive.writestr("00_executive_summary.json", json.dumps(summary, indent=2, ensure_ascii=False))
        archive.writestr("01_leed_v5_auto_report.html", html.encode("utf-8"))
